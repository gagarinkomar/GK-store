...
class RegisterForm(FlaskForm):
    name = StringField('Имя', validators=[DataRequired(), Length(
        min=3, max=10)])
    ...
    phone_number = StringField('Номер телефона', validators=[Length(
        max=20)])
    email = EmailField('Почта', validators=[DataRequired(), Length(
        min=8, max=30)])
    permission = SelectField('Роль', choices=[
        ('seller', 'Продавец'), ('buyer', 'Покупатель')])
    password_new = PasswordField('Пароль', validators=[DataRequired(), Length(
        min=5, max=35)])
...
class EditUserForm(FlaskForm):
    ...
    password_new = PasswordField('Новый пароль(не обязательно)', validators=[
        Length(max=35)])
    password_again = PasswordField('Повторите новый пароль')
    password = PasswordField('Пароль для принятия изменений', validators=[
        DataRequired()])
...
class ProductForm(FlaskForm):
    title = StringField('Заголовок', validators=[DataRequired(), Length(
        min=10, max=50)])
    short_about = TextAreaField('Краткое описание', validators=[
        DataRequired(), Length(min=15, max=75)])
    about = TextAreaField('Полное описание', validators=[
        DataRequired(), Length(min=25, max=200)])
